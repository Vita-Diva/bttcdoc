...
* private_peer_ids - 在验证人机器上设置的Heimdall的节点ID。

为了获得validator上Heimdall的节点ID。登录到validator上：

运行 heimdalld tendermint show-node-id。
...
编辑 `~/.heimdalld/config/config.toml`，在config.toml中，改变以下内容。
...
* persistent_peers - sentry机上设置的Heimdall的节点ID和sentry机的IP地址。

    为了获得Heimdall在sentry机上的节点ID。登录到sentry机上。
...
编辑 `~/.heimdalld/config/heimdall-config.toml`，在heimdall-config.toml中，修改以下内容。
...
这将生成priv_validator_key.json，将生成的JSON文件移到Heimdall配置目录中。
...
* 准备好两台机器。
* 将Heimdall和BTTC的二进制文件安装在sentry和validator上。
* 在sentry和validator上设置Heimdall和BTTC的节点文件。
* 在sentry和validator上设置Heimdall和BTTC服务。
* 配置sentry节点。
* 启动sentry节点。
* 配置validator节点。
* 设置Owner和Signer的密钥。
